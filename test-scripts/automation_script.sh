...
then
    #git clone https://$USERNAME:$PASSWORD@github.com/RutgersCSSystems/gem5-linux.git
    #git clone https://github.com/RutgersCSSystems/gem5-linux/
...
check_tlblogs_exist () {
	...
	then
		...
		#then echo "reasonable output"; fi    
		   ...
		   PID=`ps axf | grep automation_script.sh | grep -v grep | awk '{print $1}'`;kill -9 $PID
        	fi
	fi		
        sleep 1
    done
